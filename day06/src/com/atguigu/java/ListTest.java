 ...
 *            /-------ArrayList：作为List接口的主要实现类，线程不安全的，效率高；底层使用Object[]:Object[] elementData存储
 *            /-------LinkedList：对于频繁的插入、删除操作，底层使用双向链表存储
 *            /-------Vector：作为List接口的古老实现类，线程安全的，效率低；底层使用:底层使用Object[]:Object[] elementData存储
 *
 *    ArrayList的源码分析：jdk 7 和 jdk 8略有不同
 *
 ...
 *
 *      list.add(123);//第一次调用add()时，底层才创建了长度10的数组，并将数据123添加到eleementData[0]
 *      。。。
 *      后续的添加和扩容操作与jdk 7 无异
 *
 ...
 * 同：三个类都是实现了List接口，存储数组的特点相同：存储有序的、可重复的数据
 * 不同：见上
 *
 *  5.List接口中的常用方法
 */
...
总结：常用方法
增：add(Object obj)
删：remove(in index)/ remove(Object obj)
